					...
					<div class="row">
						<div class="col-md-12">
							<div class="card">
								...
								<div class="card-body">
                                    @foreach ( $blog as $row)
									<form action="{{route('admin-artikel.update',$row->id)}}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        <label for="judul Kategori">Judul</label>
                                        <input type="text" class="form-control @error('judul') is-invalid @enderror" value="{{ $row->judul }}" placeholder="Judul" name="judul">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="kategori">
                                            ...
                                            <option {{ $row->id_kategori_blog==$k->id ? 'selected' : '' }} value="{{ $k->id }}">{{ $k->nama }}</option>
                                            @endforeach
                                        </select>
